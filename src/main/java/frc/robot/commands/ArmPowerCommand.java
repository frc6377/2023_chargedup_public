...
public class ArmPowerCommand extends CommandBase {
    ...
    public void initialize (){
        initalPose = new PolarPoint(armSubsystem.thetaFromCANCoder(), armSubsystem.currentArmExtenstion());
    }

    @Override
    public void execute(){
        double armExtension = (targetPose.theta != initalPose.theta) ? computeExtension() : targetPose.r; //math breaks if theta doesnt change
    ...
    public boolean isFinished (){
        return Math.abs(armSubsystem.thetaFromCANCoder() - targetPose.theta) < Constants.ARM_ALLOWED_ANGLE_ERROR &&
            Math.abs(armSubsystem.currentArmExtenstion() - targetPose.r) < Constants.ARM_ALLOWED_EXTENSION_ERROR;
    }
