...
public class ArmCommand extends CommandBase {
  ...
  /**
   * @param targetPosition the target arm position in cartesian coordinate (+x is forwards, +y is
   *     vertical)
   * @param targetWristRotation the target wrist rotation from the ground
   * @param subsystem
   */
  public ArmCommand(
      Translation2d targetPosition, double targetWristRotation, ArmSubsystem subsystem) {
  ...
  /**
   * Computes the value needed to for the path Based on https://www.desmos.com/calculator/7w6qynhplc
   */
  private void computeInitialValues() {
    ...
    double thetaFromLineCenterToPoint =
        Math.atan2(startPosition.getY(), startPosition.getX()) - lineThetaOffset;
